...
from src.SegmentComparatorClass import SegmentComparator
import gmplot
...
class BestSegment:
    ...
    def __init__(self, referential_race_name, all_segments, mean_seg_density):
        ...
        self.deniv_segment = 0
        self.density_segment = 0
        ...
        if 1:
            ...
            for i in range(len(self.all_segments)-1):
                ...
                for seg_2 in new_all_segments:
                    ...
                    if self.mean_seg_density[name_1] < 0.5 or self.mean_seg_density[name_2] < 0.5:# si les deux densité sont plus grandes que...
                        #print("continue 1")
                        continue
                    ...
                    if len(segments_filtered) > 1:
                        print("it generates more segments -----------------------------------------", len(segments_filtered))
                        continue
                    ...
                    if segments_filtered[0].positions.shape[1] < 0.7*segment_1.shape[1] and segments_filtered[0].positions.shape[1] < 0.7*segment_2.shape[1]:
    ...
    def draw(self):
        ...
        gmap3.draw("output/" + "best_segment_" + self.referential_race_name + ".html")
